...
An OpenCAP input file has four sections; $geometry, $system, $cap_parameters, and $job.  Within each section, the user inserts keyword/value pairs which define the calculation.  All four sections have their own unique keywords, and all four must be specified to run a calculation. The general format is:
...
## CAP Parameters ##
This section allows one to specify the parameters used to define the complex absorbing potential, and defines the grid used for numerical integration.
