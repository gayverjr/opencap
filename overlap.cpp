...
double mcmurchie_davidson(double exp_a, double exp_b, int t, int angmom_a, int angmom_b, double dist)
{
	...
	{
		//decrement angmom_a
		return(1/(2*p)*mcmurchie_davidson(exp_a,exp_b,t-1,angmom_a-1,angmom_b,dist)-
	...
	{
		//decrement angmom_b
		return(1/(2*p)*mcmurchie_davidson(exp_a,exp_b,t-1,angmom_a,angmom_b-1,dist)-
				(q*dist/exp_a)*mcmurchie_davidson(exp_a,exp_b,t,angmom_a,angmom_b-1,dist)+
...
double overlap_integral(BasisFunction a, BasisFunction b)
{
	...
	for(int i=0;i<a.num_prims;i++)
	{
		for(int j=0;j<b.num_prims;j++)
		{
			...
			gauss_integral(a.exps[i],b.exps[j],a.shell,b.shell,a.center,b.center);
		}
	}
	return res;
